<template>
  <div class="content-wrapper">
      ...
      <div class="row">
        ...
        <span class="col-md-8">
            <h2>
              ...
              {{ 'sidebar.menu.admin.user.list' | message }}
              <!--<small>Preview</small>-->
            </h2>
          </span>
        <!--<ol class="breadcrumb">-->
                              ...
                              <div class="input-group-btn">
                                ...
                                <ul class="dropdown-menu">
                                  ...
                                  <li v-if="user.status === 101"><a @click="enableUser(user.nationalCode)">
                                    ...
                                    {{'common.enable' | message}}</a>
                                  </li>
                                  <li v-if="user.status === 100"><a @click="disableUser(user.nationalCode)">
                                    <i class="fa fa-ticket" aria-hidden="true"></i>
                                    {{'common.disable' | message}}</a>
                                  </li>
                                  <li><a @click="setSelectedUser(user.nationalCode)" data-toggle="modal" data-target="#resetPasswordPopup">
                                    <i class="fa fa-lock" aria-hidden="true"></i>
                                    {{'update.password' | message}}</a></li>
                                </ul>
                              </div>
                            </div>
                          </td>
    ...
    <div id="resetPasswordPopup" class="modal fade" role="dialog">
      <div class="modal-dialog">
        <lazy :loaded="resetPasswordLoaded">
          <div class="modal-content" v-if="selectedUser != null">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal">&times;</button>
              <h4 class="modal-title">{{'update.password' | message}}</h4>
            </div>
            <div class="modal-body">
              <div class="row">
            ...
            <div class="modal-footer">
              <button :disabled="resetPasswordSubmitting" type="button" class="btn btn-primary float" @click="resetPassword">{{'common.update' | message}}</button>
              <button :disabled="resetPasswordSubmitting" type="button" class="btn btn-danger float" data-dismiss="modal">{{'common.cancel' | message}}</button>
            </div>
          </div>
        </lazy>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      loadUsers: function () {
        var countUrl = '/s/adminusermangement/count'
        var listUrl = '/s/adminusermangement/list'
      ...
      setSelectedUser: function (nationalCode) {
        ...
        this.resetPasswordLoaded = false
      ...
      enableUser: function (nationalCode) {
        var enableUrl = '/s/adminusermangement/enable'
      ...
      disableUser: function (nationalCode) {
        var disableUrl = '/s/adminusermangement/disable'
        var querystring = require('querystring');
        this.submitting = true
      ...
      resetPassword: function () {
        var resetPasswordUrl = '/s/adminusermangement/resetpassword'
        var querystring = require('querystring');
          ...
          this.$validator.validateAll().then((result) => {
            if (result) {
              ...
              axios.post(resetPasswordUrl, querystring.stringify({
                ...
                confirmPassword: this.resetPasswordObj.confirmPassword
              }))
                .then(response => {
                  this.resetPasswordSubmitting = false
                  let msg = this.$options.filters.message('update.password.success')
                  this.$toasted.success(msg)
